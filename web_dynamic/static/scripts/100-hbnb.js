$(document).ready(function () {
  ...
  const data = { amenenties: Object.values(obj) };
  $('.places').empty();
  ...
  $('.locations').find('h2').find('input').on('click', function () {
    if ($(this).is(':checked')) {
      obj2[($(this).attr('data-id'))] = ($(this).attr('data-name'));
    } else {
      delete obj2[($(this).attr('data-id'))];
    }
    console.log(obj2);
    $('div.locations h4').text(Object.values(obj2));
  });
  $('.locations').find('li').find('input').on('click', function () {
    if ($(this).is(':checked')) {
      obj2[($(this).attr('data-id'))] = ($(this).attr('data-name'));
    } else {
      delete obj2[($(this).attr('data-id'))];
